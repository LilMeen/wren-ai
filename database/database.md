...
Hệ thống tuân theo mô hình **Data Lakehouse** (Medallion Architecture). Dữ liệu được chia thành **3 lớp (layer)** lưu trữ chính trên StarRocks (Hive + Iceberg catalogs), sử dụng dbt (Data Build Tool) để thực hiện quy trình ELT/ETL.

---

## 1. Lớp Raw / Bronze (Dữ liệu thô)
...
## 2. Lớp Staging / Silver (Dữ liệu trung gian / Làm sạch)
...
## 3. Lớp Golden / Gold (Dữ liệu phục vụ / Phân tích)
...
Tóm lại, việc sử dụng MinIO **không làm thay đổi logic tổ chức dữ liệu 3 lớp** (Data Lakehouse), mà chỉ thay đổi **cách hệ thống phân giải đường dẫn và quản lý metadata** ở dưới nền tảng (Infrastructure), giúp toàn bộ hệ thống "đóng gói" (containerized) được mà không phụ thuộc vào Internet.
