...
class OntologyRecommendation:
    ...
    def __init__(
        ...
        self._cache: Dict[str, OntologyRecommendation.Resource] = TTLCache(
    ...
    @observe(name="Generate Ontology Recommendation")
    @trace_metadata
    async def recommend(self, request: Input, **kwargs) -> Resource:
        logger.info("Generate Ontology Recommendation pipeline is running...")
        ...
        try:
            ...
            resp = await self._pipelines["ontology_recommendation"].run(**input)
            ...
            self._handle_exception(
                request,
                f"An error occurred during ontology recommendation generation: {str(e)}",
                trace_id=trace_id,
                request_from=request.request_from,
            )
    ...
    def __getitem__(self, id: str) -> Resource:
        ...
        if response is None:
            message = f"Ontology Recommendation Resource with ID '{id}' not found."
