...
import useAIRelationshipRecommendation from '@/hooks/useAIRelationshipRecommendation';

const { Title, Text } = Typography;

// for omit keys
const relationKeys = ['name', 'isAutoGenerated', 'reason'];
    ...
    },
    {
      ...
      render: (type, relation) => (
        <>
          ...
          {relation.reason && (
            <Tooltip title={relation.reason}>
              <InfoCircleOutlined className="ml-1 gray-6" />
            </Tooltip>
          )}
        </>
      ),
...
export default function DefineRelations(props: Props) {
  ...
  const mergeAIRelations = useCallback(
    (aiRelations: SelectedRecommendRelations) => {
      setRelations((prev) => {
        const merged = { ...prev };
        for (const [modelKey, newRels] of Object.entries(aiRelations)) {
          const existing = merged[modelKey] || [];
          // deduplicate: skip if same fromField+toField combo already exists
          const filtered = newRels.filter(
            (newRel) =>
              !existing.some(
                (e) =>
                  e.fromField.fieldId === newRel.fromField.fieldId &&
                  e.toField.fieldId === newRel.toField.fieldId,
              ),
          );
          merged[modelKey] = [...existing, ...filtered];
        }
        return merged;
      });
  ...
  const {
    ...
    elapsed,
    generate,
  } = useAIRelationshipRecommendation(mergeAIRelations);
    ...
    <div>
      ...
      <div className="mt-4">
        <Space align="center">
          ...
          {generating && (
            <Text type="secondary">
              AI is analyzing your schema to suggest relationships
              {elapsed > 0 ? ` (${elapsed}s)` : ''}...
            </Text>
          )}
        </Space>
      ...
      {showNoRecommendationAlert && (
        <Alert
          ...
          description="No relationships are recommended because no primary or foreign keys were detected. Click 'Generate with AI' to suggest relationships based on your schema."
          type="info"
          showIcon
          className="my-4"
        />
